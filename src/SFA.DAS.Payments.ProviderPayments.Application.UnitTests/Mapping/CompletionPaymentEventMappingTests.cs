...
{
    [TestFixture]
    public class CompletionPaymentEventMappingTests
    {
        ...
        public void CanMapFromPaymentModelToRecordedAct1CompletionPaymentEvent()
        {
            ...
            payment.Should().BeAssignableTo<ICompletionEvent>();
            payment.Should().BeAssignableTo<RecordedAct1CompletionPayment>();
        ...
        [Test]
        public void CanMapFromPaymentModelToRecordedShortCourseCompletionPaymentEvent()
        {
            var paymentModel = new PaymentModel
            {
                JobId = 123,
            ...
            var payment = Mapper.Map<PaymentModel, RecordedShortCourseCompletionPayment>(paymentModel);
            payment.Should().BeAssignableTo<ICompletionEvent>();
            payment.Should().BeAssignableTo<RecordedShortCourseCompletionPayment>();
            payment.EventId.Should().NotBeEmpty();
            payment.EventTime.Should().BeCloseTo(DateTimeOffset.UtcNow, TimeSpan.FromSeconds(5));
            payment.Ukprn.Should().Be(paymentModel.Ukprn);
            payment.DeliveryPeriod.Should().Be(paymentModel.DeliveryPeriod);
